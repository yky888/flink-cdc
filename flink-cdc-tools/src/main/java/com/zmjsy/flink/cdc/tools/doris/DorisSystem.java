...
// under the License.
package com.zmjsy.flink.cdc.tools.doris;
...
public class DorisSystem {
    ...
    public String buildCreateTableDDL(TableSchema schema) {
        ...
        sb.append(identifier(schema.getDatabase()))
                .append(".")
                .append(identifier(schema.getTable()).toLowerCase())
                .append("(");
    ...
    private void buildColumn(StringBuilder sql, FieldSchema field,boolean isPk){
        String typeString = field.getTypeString();
        if(isPk){
            if(DorisType.STRING.equals(typeString.toUpperCase())){
                typeString = "VARCHAR(255)";
            }
        }

        sql.append(identifier(field.getName().toLowerCase()))
                .append(" ")
                .append(typeString)
                .append(" COMMENT '")
